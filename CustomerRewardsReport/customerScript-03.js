...
const filePath = path.join(__dirname, "CustomerRewardsReport-3.csv");
...
async function runAggregationAndSaveToCSV() {
  try {
    ...
    writeStream.write(
      "Transaction Date,Business Type,Customer Join Date,Customer Full Name,Birth Date,Gender, Country Code ,Phone,Last Visit Date,Total Spending Amount,Discount,Earned Points,Current Level,Transaction Type,Bill No\n"
    );
    ...
    await cursor.forEach((item) => {
      ...
      const row = `${item.transactionDate},${item.businessType},${item.customerJoiningDate},${item.firstName} ${item.lastName},${item.birthday},${item.gender},${item?.countryCode},${item.contactNumber},${item.lastVisitDate},${item.totalSpending},${item.discount},${item.totalPoints},${item.currentLevel},${item.transactionType},${item.orderNumber}\n`;
